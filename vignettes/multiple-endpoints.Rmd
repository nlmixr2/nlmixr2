...
```{r}
fit.TOS <- cacheFit("multiple-endpoints-fit.TOS", {
  nlmixr(pk.turnover.emax3, warfarin, "saem", control=list(print=0),
...
```{r}
## FOCEi fit/vpcs
fit.TOF <- cacheFit("multiple-endpoints-fit.TOF", {
  nlmixr(pk.turnover.emax3, warfarin, "focei", control=list(print=0),
         table=list(cwres=TRUE, npde=TRUE))
})
```
