...
cmt2fit.logn <- cacheFit("xgxr-cmt2fit.logn", {
  nlmixr(
    cmt2m, data = dat2,
    est = "saem",
...
cmt2fit.prop <- cacheFit("xgxr-cmt2fit.prop", {
  cmt2fit.logn %>%
...
We can also try an add+prop error. NOTE: this fit is currently disabled
(`eval=FALSE`): on the development build its additive term collapses to zero and
the covariance step aborts (a hard C++ error that cannot be trapped). Re-enable
it once that core nlmixr2 issue is fixed.

```{r model-fit-2cmt-addprop, eval=FALSE}
## now try add+prop
cmt2fit.add.prop <- cacheFit("xgxr-cmt2fit.add.prop", {
  cmt2fit.prop %>%
...
Now that we have run the estimation methods, we can compare the
results side-by-side (add+prop omitted while the fit above is disabled).
