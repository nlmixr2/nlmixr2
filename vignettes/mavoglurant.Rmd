...
```{r saem}
fit.addProp.S <- cacheFit("mavoglurant-fit.addProp.S", {
  nlmixr(pbpk, dat, est="saem", control=list(print=0),
...
```{r saemLogNormal}
fit.lnorm.S <- cacheFit("mavoglurant-fit.lnorm.S", {
  pbpk %>%
...
```{r focei}
fit.addProp.F <- cacheFit("mavoglurant-fit.addProp.F", {
  fit.addProp.S %>%
    nlmixr(est="focei",
...
## Log-normal
```{r logNormalFocei}
fit.lnorm.F <- cacheFit("mavoglurant-fit.lnorm.F", {
  fit.addProp.F %>%
...
fit.lnorm.trans <- cacheFit("mavoglurant-fit.lnorm.trans", {
  pbpkL %>%
