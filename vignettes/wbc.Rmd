...
fit.S <- cacheFit("wbc-fit.S", {
  nlmixr(wbc, d3, est="saem", list(print=0), table=list(cwres=TRUE, npde=TRUE))
})
...
```{r focei}
fit.F <- cacheFit("wbc-fit.F", {
  nlmixr(wbc, d3, est="focei", list(print=0), table=list(cwres=TRUE, npde=TRUE))
})
```
