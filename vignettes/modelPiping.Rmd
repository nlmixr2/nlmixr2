...
one.ka.0.5 <- cacheFit("modelPiping-one.ka.0.5-fix", {
  fit %>%
    ini(tka=fix(0.5)) %>%
...
one.ka.0.5 <- cacheFit("modelPiping-one.ka.0.5-fixEst", {
  fit %>%
    ini(tka=fix) %>%
    nlmixr(est="focei", control=list(print=0),
           table=list(cwres=TRUE, npde=TRUE))
...
one.ka.0.7 <- cacheFit("modelPiping-one.ka.0.7", {
  one.compartment %>%
    ini(tka=0.7) %>%
...
```{r remove-eta.ka}
## Remove eta.ka on ka
noEta <- cacheFit("modelPiping-noEta", {
  fit %>%
    model(ka <- exp(tka)) %>%
    nlmixr(est="focei", control=list(print=0),
           table=list(cwres=TRUE, npde=TRUE))
...
```{r add-bsv.ka}
addBackKa <- cacheFit("modelPiping-addBackKa", {
  noEta %>%
...
wt70 <- cacheFit("modelPiping-wt70", {
  fit %>%
...
addPropModel <- cacheFit("modelPiping-addPropModel", {
  fit %>%
